function varargout = HighPassFilter_contentview(varargin)
% HIGHPASSFILTER_CONTENTVIEW MATLAB code for HighPassFilter_contentview.fig
%      HIGHPASSFILTER_CONTENTVIEW, by itself, creates a new HIGHPASSFILTER_CONTENTVIEW or raises the existing
%      singleton*.
%
%      H = HIGHPASSFILTER_CONTENTVIEW returns the handle to a new HIGHPASSFILTER_CONTENTVIEW or the handle to
%      the existing singleton*.
%
%      HIGHPASSFILTER_CONTENTVIEW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HIGHPASSFILTER_CONTENTVIEW.M with the given input arguments.
%
%      HIGHPASSFILTER_CONTENTVIEW('Property','Value',...) creates a new HIGHPASSFILTER_CONTENTVIEW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HighPassFilter_contentview_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HighPassFilter_contentview_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HighPassFilter_contentview

% Last Modified by GUIDE v2.5 11-Jun-2017 14:51:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HighPassFilter_contentview_OpeningFcn, ...
                   'gui_OutputFcn',  @HighPassFilter_contentview_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before HighPassFilter_contentview is made visible.
function HighPassFilter_contentview_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HighPassFilter_contentview (see VARARGIN)

% Choose default command line output for HighPassFilter_contentview
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes HighPassFilter_contentview wait for user response (see UIRESUME)
...
set(handles.text40,'Units','normalized')
...
set(handles.text45,'Units','normalized')
set(handles.text46,'Units','normalized')
set(handles.text47,'Units','normalized')
set(handles.text48,'Units','normalized')
set(handles.text49,'Units','normalized')
set(handles.text50,'Units','normalized')

% --- Outputs from this function are returned to the command line.
function varargout = HighPassFilter_contentview_OutputFcn(hObject, eventdata, handles) 
...
af= 1+(r2/r1);
fc=1/(2*pi*sqrt(r1*r2*(c1*1e-6*c2*1e-6)));
vo=(vi*af*(fi/fc))/(sqrt(1+(fi/fc)^2));
g=20*log10(vo/vi);

set(handles.text10,'string',fc);
set(handles.text11,'string',vo);
set(handles.text12,'string',af);
set(handles.text13,'string',g);
...
title('Sinyal Masukan High Pass Filter')
axis([0 inf -12 12])
...
title('Sinyal Keluaran High Pass Filter')
axis([0 inf -12 12])

f=0:10:fi;
vo1=(vi*af*(f/fc))./(sqrt(1+(f/fc).^2));
g1=20*log10(vo1/vi);
axes(handles.axes4)
plot(f,g1,'r','LineWidth',2.5);
title('Karakteristik High Pass Filter');
